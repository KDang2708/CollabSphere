...
def tao_nhiem_vu():
    ...
    if not du_lieu or "id" not in du_lieu or "id_du_an" not in du_lieu or "ten_nhiem_vu" not in du_lieu:
...
def phan_cong_nhiem_vu(id_nhiem_vu):
    du_lieu = request.get_json()
    if not du_lieu or "nguoi_thuc_hien" not in du_lieu:
...
def cap_nhat_trang_thai(id_nhiem_vu):
    du_lieu = request.get_json()
    if not du_lieu or "trang_thai" not in du_lieu:
